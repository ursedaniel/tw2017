...
export class DocsService {
  ...
  private urlSearch = "/api/docs/search?sort=id,asc";
  private urlDocs = "/api/docs";

  constructor(private http: Http) {
    this.http = http;
  }

  getDocs(searchParamas:SearchParams): Observable<JsonObject> {
    this.setSearch(searchParamas);
    return this.http.get(this.urlDocs,{search:this.search}).map((response: Response) => <JsonObject>response.json());
  }

  getFilteredDocs(searchParamas:SearchParams): Observable<JsonObject> {
    this.setSearch(searchParamas);
